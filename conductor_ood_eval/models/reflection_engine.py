...
class ReflectionEngine (ModelEngine ):
    ...
    def __init__ (self ,config :ModelConfig ):
        ...
        self .is_ready =False 
        self .model_one =config .config .get ('model_one','meta-llama/Llama-3.1-8B-Instruct')
        self .model_two =config .config .get ('model_two','meta-llama/Llama-3.1-8B-Instruct')
    ...
    async def generate (self ,request :GenerationRequest )->GenerationResponse :
        ...
        logger .info (f"First agent response: {q1_res[0]}")

        refl_res =await self ._run_reflect (
        model =self .model_two ,
        ...
        logger .info (f"Reflection response: {refl_res}")

        prompt_tokens =sum (len (msg ["content"].split ())for msg in request .messages )
        completion_tokens =len (refl_res .split ())
